  ...
  <div class="m-t-20px m-x-10px">
    ...
    >
      <div class="flex items-center flex-wrap gap-6px text-16px m-r-16px">
        <div
          class="p-x-6px p-y-8px text-#464646 bg-[rgba(0,0,0,0.06)] dark:text-#FAFAFA font-bold hover:text-#FAFAFA hover:bg-[--el-color-primary-light-3] rounded-8px whitespace-nowrap line-clamp-1 select-none"
          v-for="item in tagList"
          :key="item.tagId"
          @click="handleArticleTag(item.tagId)"
          :class="{
            'text-#FAFAFA! bg-[--el-color-primary]!': tagId == item.tagId
          }"
        >
          {{ item.tagNamePlus }}
        </div>
...
/** 点击文章标签 */
const tagId = ref("0");
const handleArticleTag = (value: string) => {
  tagId.value = value;
...
onMounted(() => {
  routeParam.value = route.params.tagId || "";
  searchParams.value.tagId = routeParam.value;
  tagId.value = routeParam.value;
  handleListTag();
  handleListPage();
});
