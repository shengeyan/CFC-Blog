<template>
  ...
  <el-tooltip content="明亮模式" v-if="!globalStore.isDark">
    <SvgIcon name="koi-menu-sun" width="20px" height="20px" class="rounded-full p-5px bg-[rgba(50,50,50,0.06)] m-r-10px border-none outline-none" @click="handleSwitchDark"></SvgIcon>
  </el-tooltip>
  <!-- 暗黑模式 -->
  <el-tooltip content="暗黑模式" v-if="globalStore.isDark">
    <SvgIcon name="koi-menu-moon" width="20px" height="20px" class="rounded-full p-5px bg-[rgba(50,50,50,0.06)] m-r-10px border-none outline-none" @click="handleSwitchDark"></SvgIcon>
  </el-tooltip>
</template>
