<template>
  <!-- 头部 header 开始 #141414 #37383A -->
  <header class="w-100% fixed z-999 top-0 left-0 select-none dark:bg-[rgba(50,51,53,0.8)] header">
    <!-- 头部内置box开始 bg-#F6F9F -->
    <div class="max-w-1600px h-50px m-y-0 m-x-auto p-x-15px p-y-8px">
      <!-- LOGO开始 -->
      <KoiLogo></KoiLogo>
      <!-- LOGO结束 -->
      <!-- 工具栏开始 -->
      <div class="relative float-right p-r-18px p-t-8px">
        <div class="flex items-center">
          <!-- 个人信息 -->
          <KoiMine></KoiMine>
          <!-- 搜索文章 -->
          <KoiSearchArticle></KoiSearchArticle>
          <span class="c-[rgba(50,50,50,0.5)] m-t-2px m-x-10px dark:c-#8D8D8D"> | </span>
          <!-- 明亮/暗黑模式 -->
          <KoiDark></KoiDark>
          <!-- 发布文章 -->
          <KoiPublish></KoiPublish>
          <!-- 更多菜单 -->
          <KoiMore class="m-t-6px"></KoiMore>
        </div>
      </div>
      <!-- 工具栏结束 -->
    </div>
    <!-- 头部内置box结束  -->
  </header>
  <!-- 头部 header 结束 -->
</template>

<script setup lang="ts">
import { ref } from "vue";
import KoiLogo from "./Logo.vue";
import KoiDark from "./Dark.vue";
import KoiPublish from "./Publish.vue";
import KoiSearchArticle from "./GlobalSearch.vue";
import KoiMine from "./Mine.vue";
import KoiMore from "./More.vue";

const headerBackground = ref("linear-gradient(0, rgba(0, 0, 0, 0) 5%, rgba(0, 0, 0, 0.1) 80%)");
window.onscroll = function () {
  if (window.scrollY <= 0 || document.documentElement.scrollTop <= 0) {
    // 如果滚动回到了顶部，0代表0px
    // 恢复原始背景颜色
    headerBackground.value = "linear-gradient(0, rgba(0, 0, 0, 0) 5%, rgba(0, 0, 0, 0.1) 80%)";
  } else {
    headerBackground.value = "blur(10px)";
  }
};
</script>

<style lang="scss" scoped>
/** 头部样式 */
.header {
  background: v-bind(headerBackground);
  backdrop-filter: v-bind(headerBackground);
}
</style>
